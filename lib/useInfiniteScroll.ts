...
const useInfiniteScroll = (pageInfo: any, itemClass: string) => {
  ...
  const handleScroll = () => {
    const lastProduct = document.querySelector(`.${itemClass}:last-child`) as HTMLElement

    if (lastProduct) {
      const lastTestimonialOffset = lastProduct.offsetTop + lastProduct.clientHeight
      const pageOffset = window.pageYOffset + window.innerHeight

      if (pageOffset > lastTestimonialOffset && currPage < totalPage - 1) setCurrPage(currPage + 1)
    }
  }
